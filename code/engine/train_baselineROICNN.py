...
from utils import get, saveModel
from placeholders.shared_placeholders import *
from itertools import product
from DataPlotter import PlotTrainingValidationLoss
...
def SaveModel(sess, saver, path, stepSize=100):
    """
...
def TrainModel(sess, splitTrainSet, splitValidationSet, matricesPL, labelsPL, predictionLayer, trainOperation, lossFunction, savePath,
    ...
    """
    ############# Define tf saver #############
    saver = save_path = saveModel.restore(sess, savePath)
    ...
    for batch_index in range(numberOfSteps):
        ...
        trainingLoss, validationLoss = ReportProgress(sess, batch_index, lossFunction, matricesPL, labelsPL, splitTrainSet, splitValidationSet)
        accumulatedTrainingLoss.append(trainingLoss)
        accumulatedValidationLoss.append(validationLoss)

        ############# SAVE TRAINED MODEL #############
        SaveModel(sess, saver, savePath)

    return (accumulatedTrainingLoss, accumulatedValidationLoss)

def CrossValidateModeLParameters(sess, splitTrainSet, matricesPL, labelsPL, predictionLayer, trainOperation, lossFunction, savePath, saveName,
                                 numberOfSteps=get('TRAIN.ROI_BASELINE.NB_STEPS'), batchSize=get('TRAIN.ROI_BASELINE.BATCH_SIZE')):
    """
    ...
    for tIndex, vIndex in folder.split(X):
        ...
        foldTrainingLosses, foldValidationLosses = TrainModel(sess, splitTrainSet, splitValidationSet,
                                                    matricesPL, labelsPL, predictionLayer, trainOperation,
...
def CrossValidateModelParameters(dataSet, matrixPlaceholders, labelPlaceholders, predictionLayers, trainOperations,
                                 lossFunction, stepCounts, batchSizes, saveNames):
    ...
    for matricesPL, labelsPL, predictionLayer,
        trainOperation, numberOfSteps, batchSize in
        product(matrixPlaceholders, labelPlaceholders, predictionLayers,
                trainOperations, stepCounts, batchSizes):

        saveName = saveNames[index]
        averageFinalValidationPerformance = CrossValidateModeLParameters(sess, splitTrainSet,
                                                matricesPL, labelsPL, predictionLayer, trainOperation,
