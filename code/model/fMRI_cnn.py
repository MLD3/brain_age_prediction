...
def fMRI_4D_CNN(meanDefault=0.0, sdDefault=0.01, biasDefault=0.1):
    ...
    # 13*16*12*2 -> 13*16*12*8
    conv2 = conv_layer(x=pool1, input_height = 13, input_width = 16, input_depth = 12, input_channels=2,
                        filter_size=7, output_channels=8, padding='SAME',
                        act_type='relu', mean=meanDefault, sd=sdDefault, bias=biasDefault, stride=1)
    # 13*16*12*8 -> 7*8*6*8
    pool2 = pool_layer(x=conv2, input_height = 13, input_width = 16, input_depth = 12, input_channels=8, k_length=2, stride=2, padding='SAME')
    # 7*6*8*8 -> 7*8*6*16
    conv3 = conv_layer(x=pool2, input_height = 7, input_width = 8, input_depth = 6, input_channels=8,
                        filter_size=3, output_channels=16, padding='SAME',
                        act_type='relu', mean=meanDefault, sd=sdDefault, bias=biasDefault, stride=1)
    # 7*8*6*16 -> 4*4*3*16
    pool3 = pool_layer(x=conv3, input_height = 7, input_width = 8, input_depth = 6, input_channels=16, k_length=3, stride=2, padding='SAME')
    print(pool3.shape)
    # fully connected
    fn1 = fully_connected_layer(pool3, shape=[4 * 4 * 3 * 16, 1], act_type='none', mean=meanDefault, sd=sdDefault, bias=biasDefault)
